/* PROTOTYPE — l'écran « domaines » en îlot React, À CÔTÉ de domaines.astro.
 *
...
const LS = 'mordu_vus_proto_react';   // clé DISTINCTE : le prototype ne touche pas l'état réel
...
function Carte({ d, vus, onOuvrir }) {
  ...
  return (
    <div className={'dom apparait' + (m >= 0.999 ? ' done' : '')} onClick={onOuvrir}>
      <div className="frame">
        <Vignette path={d.image_path} net={m} type={d.type} />
        <span className="tag">{ETIQ[d.type]}</span>
      </div>
      <div className="name">{d.name}</div>
      <div className="m"><i style={{ width: Math.round(m * 100) + '%' }} /></div>
      <div className="mv">{dedans}/{d.catalogue_ids.length} vus · {d.canon_size} essentiels</div>
    </div>
  );
}
...
function Modale({ dom, vus, basculer, fermer }) {
  ...
  return (
    <div className="modal on" onClick={(e) => e.target === e.currentTarget && fermer()}>
      <div className="sheet">
        {plein === null && <p className="empty">chargement…</p>}
        {plein === false && <p className="empty">erreur</p>}
        {plein && (
          <>
            <button className="close" onClick={fermer}>fermer ✕</button>
            <h3>{plein.name}</h3>
            <div className="sub">
            ...
            <div className="films">
              {plein.canon.map((f) => {
                ...
                return (
                  <div
                    key={f.id}
                    className={'film ' + (a ? (vu ? 'seen' : '') : 'off na')}
                    onClick={() => a && basculer(f.id)}
                  >
                    {f.poster_url
                      ? <img src={f.poster_url} alt="" loading="lazy" />
                      : <div className="ph" />}
                    <span className="chk">{vu ? '✓ vu' : 'vu ?'}</span>
                    <div className="ft">{f.title}</div>
                    <div className="fy">{f.year || ''}{a ? '' : ' · hors MORDU'}</div>
                  </div>
                );
              })}
            </div>
...
export default function DomainesIlot() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error('[domaines-react]', e);
  ...
  if (erreur) return <div className="banner"><b>{erreur}</b> — lance <code>uvicorn main:app --reload</code> et recharge.</div>;
